...
interface UpdateCoursePopupProps {
  onClose: (...args: unknown[]) => unknown;
  /**
   * Émise à la sauvegarde avec le cours modifié ; le parent persiste comme il veut.
   * Peut être async (POST/PUT) : le popup attend sa résolution avant de se fermer,
   * et reste ouvert en affichant une erreur si elle rejette.
   */
  onSave: (course: CourseUpdate) => MaybePromise<unknown>;
  /** Cours à éditer (pré-remplit le formulaire). */
  course: CourseData;
  /** Programmes sélectionnables, fournis par le parent. */
  programs?: Program[];
  /** Surcharge des textes ; seuls les champs fournis remplacent les défauts. */
  labels?: Partial<UpdateCoursePopupLabels>;
}
 ...
 */
const defaultLabels: UpdateCoursePopupLabels = {
...
export function UpdateCoursePopup({
  ...
  programs = [],
  labels,
}: UpdateCoursePopupProps): React.ReactElement {
  const t = { ...defaultLabels, ...labels };

  // Champs du formulaire, pré-remplis avec le cours existant.
  const [code, setCode] = useState(course.code);
  const [title, setTitle] = useState(course.title);
  /** Ids des programmes rattachés au cours. */
  const [programIds, setProgramIds] = useState<number[]>(course.programIds);
  ...
  async function save() {
    ...
    const update: CourseUpdate = { title: title.trim(), code: code.trim(), programIds };
