// Préfixe /api appliqué à TOUS les contrôleurs REST sauf le package `realtime`.
//
// On n'utilise PAS spring.mvc.servlet.path=/api : ce réglage déplacerait aussi
// l'endpoint WebSocket (/ws) sous /api. Ici le DispatcherServlet reste à la racine,
// donc /ws garde son chemin ; seuls les @RestController hors du package `realtime`
// reçoivent /api (ex. /programs → /api/programs, /me → /api/me).
//
...
public class WebMvcConfig implements WebMvcConfigurer {
  ...
  public void configurePathMatch(PathMatchConfigurer configurer) {
    ...
    // SAUF le package realtime (WebSocket /ws) et les endpoints INTERNES (/internal/**).
    //
