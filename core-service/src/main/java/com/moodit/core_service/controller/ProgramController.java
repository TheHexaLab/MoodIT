...
public class ProgramController {
    ...
    public ResponseEntity<CourseDTO> getCourseByProgram(@PathVariable Integer programId, @PathVariable Integer courseId) {
    ...
    @PostMapping("/courses")
    public ResponseEntity<CourseDTO> addCourseToPrograms(@RequestBody CourseCreateInProgramsDTO courseCreateDTO) {
        return ResponseEntity.ok(programService.addCourseToPrograms(courseCreateDTO));
    }

    //Ajouter un usager dans un programme
    @PostMapping("/users")
    public ResponseEntity<Void> addUserToPrograms(@RequestBody UserCreateInProgramsDTO userCreateDTO) {
